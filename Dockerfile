...
RUN    git clone https://github.com/fireice-uk/xmr-stak.git \
    ...
    && cmake -DCUDA_ENABLE=OFF -DOpenCL_ENABLE=OFF -DCMAKE_C_FLAGS="-mno-avx -march=x86-64 -mtune=generic -ggdb" -DCMAKE_CXX_FLAGS="-mno-avx -march=x86-64 -mtune=generic -ggdb" . \
    && make -j$(nproc) \
...
RUN ls -la /usr/local/bin/xmr-stak
